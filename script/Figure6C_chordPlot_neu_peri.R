...
perToNeu_Df = as.matrix(read.csv(paste0(figure6Folder, '/perToNeu_Df.csv'), row.names = 1))
perToNeu_Df_T = as.matrix(read.csv(paste0(figure6Folder, '/perToNeu_Df_T.csv'), row.names = 1))
...
# #create a chord diagram but without labeling 
# circos.par(start.degree = 90, clock.wise = FALSE )
# # chordDiagram(as.matrix(neuToPer_Df_T), annotationTrack = "grid", preAllocateTracks = 1, big.gap = 30)
# chordDiagram(neuToPer_Df_T, annotationTrackHeight = c(0.03, 0.01), annotationTrack = "grid", preAllocateTracks = 1, big.gap = 20)

#create a chord diagram but without labeling 
circos.par(start.degree = 270 ) # clock.wise = FALSE # track.margin=c(0,0)
# chordDiagram(as.matrix(neuToPer_Df_T), annotationTrack = "grid", preAllocateTracks = 1, big.gap = 30)
chordDiagram(neuToPer_Df_T, annotationTrackHeight = c(0.03, 0.01), annotationTrack = "grid", preAllocateTracks = 1, big.gap = 20)
# chordDiagram(neuToPer_Df_T, annotationTrack = "grid", preAllocateTracks = list(track.height = max(strwidth(unlist(dimnames(mat))))))
...
#saving the plot (high definition)
# dev.copy(png, paste0(figure6Folder, '/pic/neuToPer_Df_T.png'), width=16, height=16, units="in", res=600)
dev.copy(pdf, paste0(figure6Folder, '/pic/neuToPer_Df_T.pdf'), width=16, height=16, ) #units="in", res=500
dev.off()
circos.clear()
...
#create a chord diagram but without labeling 
circos.par(start.degree = 270 ) # clock.wise = FALSE # track.margin=c(0,0)
# chordDiagram(as.matrix(perToNeu_Df), annotationTrack = "grid", preAllocateTracks = 1, big.gap = 30)
chordDiagram(perToNeu_Df, annotationTrackHeight = c(0.03, 0.01), annotationTrack = "grid", preAllocateTracks = 1, big.gap = 20)
# chordDiagram(perToNeu_Df, annotationTrack = "grid", preAllocateTracks = list(track.height = max(strwidth(unlist(dimnames(mat))))))
...
#saving the plot (high definition)
# dev.copy(png, paste0(figure6Folder, '/pic/perToNeu_Df.png'), width=16, height=16, units="in", res=600)
dev.copy(pdf, paste0(figure6Folder, '/pic/perToNeu_Df.pdf'), width=16, height=16, ) #units="in", res=500
dev.off()
circos.clear()
