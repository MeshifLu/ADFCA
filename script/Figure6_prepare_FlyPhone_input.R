...
figure6Folder='Analysis/Figure/Figure6'
dir.create(figure6Folder, showWarnings = FALSE)
...
seurat_gt
saveRDS(seurat_gt, file = paste0(figure6Folder, '/seuratObj_', 'control', '.rds'))
seurat_gt <- subset(x = seurat_concat, subset = genotype  == 'control' & age %in% c('10', '20'))
seurat_gt
saveRDS(seurat_gt, file = paste0(figure6Folder, '/seuratObj_', 'control_inAB42', '.rds'))
seurat_gt <- subset(x = seurat_concat, subset = genotype  == 'control' & age %in% c('20', '30'))
seurat_gt
saveRDS(seurat_gt, file = paste0(figure6Folder, '/seuratObj_', 'control_inTau', '.rds'))

# AB42
seurat_gt <- subset(x = seurat_concat, subset = genotype  == 'AB42')
saveRDS(seurat_gt, file = paste0(figure6Folder, '/seuratObj_', 'AB42', '.rds'))
# hTau
seurat_gt <- subset(x = seurat_concat, subset = genotype  == 'hTau')
saveRDS(seurat_gt, file = paste0(figure6Folder, '/seuratObj_', 'hTau', '.rds'))
