...
figureCCCFolder <- paste(projectFolder, projectAnalysisFoler, 'Figure', 'FigureCCC', sep = '/')
figureCCCFolder
...
neuToPer_Df = as.matrix(read.csv(paste0(figureCCCFolder, '/neuToPer_Df.csv'), row.names = 1))
neuToPer_Df_T = as.matrix(read.csv(paste0(figureCCCFolder, '/neuToPer_Df_T.csv'), row.names = 1))
...
# dev.copy(png, paste0(figureCCCFolder, '/pic/neuToPer_Df.png'), width=16, height=16, units="in", res=600)
dev.copy(pdf, paste0(figureCCCFolder, '/pic/neuToPer_Df.pdf'), width=16, height=16, ) #units="in", res=500
dev.off()
circos.clear()
